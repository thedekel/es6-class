...
assert.ok(
  recast.Syntax.hasOwnProperty('ClassExpression'),
  'current version of recast provides a version of esprima that does not ' +
    'support es6 classes! please use esprima-fb or equivalent.'
);
...
function visitClassDeclaration(nodePath) {
  ...
  nodePath.replace(b.variableDeclaration(
    ...
    [b.variableDeclarator(
      node.id,
      generateClassDefinitionExpression.call(this, nodePath)
    )]
  ));
}
...
function visitClassExpression(nodePath) {
  var node = nodePath.value;
  nodePath.replace(generateClassDefinitionExpression.call(this, nodePath));
}
 ...
 * @this {ast-types.NodePath}
 */
function generateClassDefinitionExpression(nodePath) {
  var node = nodePath.value;
...
function visitSuperCall(nodePath) {
  ...
  if (classNode && classNode.CLASS_ID && methodDefinition) {
...
function visitSuperMemberExpression(nodePath) {
  ...
  if (classNode && classNode.CLASS_ID) {
...
module.exports.parse = function(source) {
  return transform(recast.parse(source));
};
